...
- **`serverExternalPackages` + ESM 의존성 → Node 버전이 핵심 (`engines.node: "22.x"` 필수)** → externalize 된 패키지는 런타임에 `require()` 로 로드되는데, 그 의존성에 ESM-only 모듈이 있으면 **Node <22.12 에서 `ERR_REQUIRE_ESM`** 으로 `/api/inngest` 가 import 단계에서 500 크래시한다. **로컬 Node 22.18 은 `require(ESM)` 을 지원해 통과하지만, Vercel 이 기본 Node 20 을 쓰면 배포에서만 터진다**(시크릿 창에서도 500 = 보호 문제 아님, 순수 런타임 크래시). **해결: [package.json](package.json) `engines.node: "22.x"` 고정 + Vercel 프로젝트 Settings → Build and Deployment → Node.js Version 도 22.x.** (pdfjs-dist·whatwg-encoding@3 등 crawler 그래프의 ESM 의존성이 이 경로로 걸린다.) 진단: 로컬 `pnpm build && pnpm start` 후 `/api/inngest` 가 401 이면 코드는 정상 → Vercel 만 500 이면 Node 버전 의심.
- **`jsdom` 제거 → `linkedom` 사용** → jsdom 의 transitive `html-encoding-sniffer@6`(+`@exodus/bytes`) 가 ESM-only 라 위 `ERR_REQUIRE_ESM` 의 첫 트리거였다. [html.ts](src/lib/crawler/html.ts) 의 Readability DOM 을 `linkedom` `parseHTML` 로 공급(`new Readability(document)`). cheerio 는 DOM 스펙이 아니라 Readability 대체 불가. linkedom 은 번들 친화적이라 `serverExternalPackages` 에 안 넣음. (jsdom 제거로 의존성도 가벼워짐 — 다만 Node 버전 고정이 더 근본 해결.)
