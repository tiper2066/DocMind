...
| 항목 | 값 |
|---|---|
| **활성 Phase** | Phase 9 — 데모 준비 & 최종 검증 (**개발 작업 완료 → 라이브 검증 단계**) |
| **완료 Phase** | Phase 0, Phase 1, Phase 2, Phase 3, Phase 4, Phase 5, Phase 6, Phase 7, Phase 8 |
| **다음 액션** | **개발(코드) 산출물은 마무리됨** — 배포 완료(`penta-mind5.vercel.app`, Inngest `trend-scan` 포함 sync 확인). 잔여는 **배포 사이트에서 직접 기능 실행하며 진행**(데모팀): 리허설 7항목 라이브 검증, Demo A/B 리허설(<3분/<2분), 백업 mp4 녹화·`public/demo/*.mp4` 배치, WCAG AA 대비 감사, Sentry 알람룰, seed-demo `DEMO_FILES` 실제 URL. 선택: 사전생성 인터뷰 fallback 캐시(미구현 — 백업은 `/demo/playback` 녹화 재생), KB 다양화(WAPPLES 편중 — trend 수집으로 일부 해소됨). |
| **마지막 갱신** | 2026-06-18 (**UI-only 데모 안내 배너** — 앱 최상단에 상시 배너([DemoBanner](src/components/DemoBanner.tsx), `UI_ONLY` 일 때만 렌더, [(app)/layout](<src/app/(app)/layout.tsx>) TopNav 위)로 "현재 API 연결 해제 상태로 UI 둘러보기만 가능(AI 기능 임시 중단)" 문구 노출 — 접속자가 데모 열람 전용임을 즉시 인지. amber 톤·Info 아이콘·다크모드 대응. lint·build PASS.) · 2026-06-18 (**UI-only 데모 모드 전환 — 비용/만료 서비스 연결 해제** — 해커톤 발표·데모 종료 후 Anthropic·Voyage·Sentry(트라이얼)·Inngest 연결을 끊고 기존 데이터로 화면만 둘러보게 함. 단일 플래그 `DEMO_UI_ONLY=1`([demo-mode.ts](src/lib/demo-mode.ts))로 단락: 인터뷰=고정 질문/답변([service.ts](src/lib/interview/service.ts) `CANNED_QUESTIONS`), 문서 생성=답변 반영 고정 덱([generate.ts](src/lib/ppt/generate.ts) `cannedPlan`/`cannedSlide`, 조립부·미리보기·.pptx 경로 그대로 재사용), Inngest 발화=no-op([client.ts](src/inngest/client.ts) `dispatch` 래퍼 + 6개 라우트 교체). **키 제거 가능**하도록 [anthropic.ts](src/lib/anthropic.ts)(Proxy)·[embeddings.ts](src/lib/embeddings.ts)(callVoyage 내부 검증) 지연 초기화. Sentry 는 [next.config.ts](next.config.ts) `withSentryConfig` 제거 + 3개 init 을 DSN env 가드로(파일 보존). Supabase·Google 로그인 유지. `.env.local` 에서 `DEMO_UI_ONLY=1` + 4종 키 비우면 됨(`0`/제거 시 복원). 검증: lint PASS, 키 비운 채 `pnpm build` PASS. §8 함정·Plan 16장. 별건: corepack pnpm 다운로드 프롬프트 해소 위해 [package.json](package.json) `packageManager: pnpm@11.7.0` 고정.) · 2026-06-12 (**KB 트렌드 기능 숨김 설정(관리자 전용)** — 설정 페이지 우측에 "최신 지식 및 동향 숨김" 토글([TrendFeatureCard](src/components/settings/TrendFeatureCard.tsx)) — tiper@pentasecurity.com 하드코딩([trend-admin.ts](src/lib/trend-admin.ts) `TREND_FEATURE_ADMIN_EMAIL`) 계정에만 카드 렌더, 효과는 전체 사용자. 숨김 ON = KB 스위치·trend 탭·수집 카드 비노출 + `auto_run` 강제 OFF(수집 중지), 숨김 중 스위치 ON 직접 호출은 409. **숨김 해제는 스위치 OFF 로 복원** — 수집 재개는 스위치 재ON 필요. 영속화 `agents(kind='trend').config_json.featureHidden`(마이그레이션 없음). 수집된 trend 소스는 UI 만 숨김(RAG 참여 유지, 의도됨). lint·build PASS. 상세 Plan 16장.) · 2026-06-12 (**버전 비교 하이브리드 + 발표자 잠금 툴팁** — ① 문서 상세 버전 비교를 텍스트 diff 단일 → **슬라이드 쌍 좌우 비교 + 변경 쌍 아래 라인 diff**([VersionCompare](src/components/docs/VersionCompare.tsx), [diff.ts](src/lib/diff.ts) `diffSlidePairs` 슬라이드 단위 LCS 정렬·`slideLines` 번호 분리, `deckToLines` 출력 불변). 연속 동일 슬라이드는 "동일한 슬라이드 N장 생략" 한 줄, `DeckSchema.safeParse` 실패 시 기존 텍스트 diff 폴백. **모바일(md 미만)**은 하단 섹션 숨기고 타임라인 base 카드 바로 아래 인라인 텍스트 전용 diff(`VersionCompareText`, 신규/삭제는 +/− 합성) + 단일 버전 안내. ② 발표자 화이트리스트(`TREND_ADMIN_EMAILS`/`APPROVAL_ADMIN_EMAILS`) 밖 사용자의 비활성 버튼 5곳(TrendSwitch·DetectButton·ApprovalActions·PublishAllBar·BulkApproveCard)에 hover 툴팁 "데모버전이며 발표자만 작동가능합니다."([PresenterLockTooltip](src/components/PresenterLockTooltip.tsx) — 네이티브 title→Tooltip 컴포넌트, 문구 통일, §8 함정). lint·build PASS. 상세 Plan 16장.) · 2026-06-11 (**승인 게이트 이동 — 감지→승인→잔여 4단계** — 루프가 감지에서 멈추고 `approvals(kind='regenerate')` 카드 생성, "발행 승인" 클릭 시 인식→판단→행동→학습 진행 + **자동 발행·Slack 발송**(`approvedPublish` 플래그 관통), "승인 거부" 시 갱신 중단 + 우측 "최근 거부" 뱃지. 버전 비교는 승인 후 생성. [force-change.ts](scripts/force-change.ts) 2단 폴링 재작성(KEEP_PENDING=감지 대기 카드 유지). 격리 로컬 e2e PASS(감지 2.2s 멈춤→승인→20.5s 완주·v7 published·slack sent). prod 반영 push 는 완료됨(2026-06-12 확인, `ffc7f81` 까지 origin/main·Vercel 자동 배포). 상세 Plan 16장.) · 2026-06-11 (**개발 마무리** — 커밋 3건(PPT 디자인 개선 / KB trend+페이지네이션+원문링크 / docs) push·배포 완료, 배포 검증 통과(`/api/inngest` 401, Inngest Cloud `trend-scan` 등록). Phase 9 잔여는 배포 사이트 라이브 검증으로 전환 — Plan 15장 Phase 9 헤더 참조. [PRODUCT_DEV_DOC.md](docs/PRODUCT_DEV_DOC.md) 산출물 문서 작성.) · 2026-06-11 (**KB "최신 지식 및 동향" 자동 수집** — KB 헤더 우측 스위치 ON 동안 AI 가 KB 주제 기반 웹 검색(Anthropic web search 서버 도구)으로 관련 최신 URL 자동 수집 → 세 번째 탭에 동일 카드 표시. Inngest [trend-scan](src/inngest/trend.ts): cron `TZ=Asia/Seoul 0 0,12 * * *` + **스위치 ON 시 즉시 1회**([PATCH /api/kb/trend](src/app/api/kb/trend/route.ts)). **성공분만 등록**(크롤 성공 시에만 insert → 실패 카드 불가, maxPerRun=20 은 성공 수 기준), 억지 충원 금지, URL dedup, 30일 자동 정리. `sources.origin` 컬럼(마이그레이션 0004, 적용 완료). trend 소스는 **변경 감지 cron 제외**(승인 큐 노이즈 방지), RAG 는 정상 참여. [TrendSwitch](src/components/kb/TrendSwitch.tsx). **후속**: 카드/시트 원문 열기 링크, 서버 페이지네이션([SourcePagination](src/components/kb/SourcePagination.tsx), 12장/페이지, 3탭 공통, `?tab=&p_*=`), **워크스페이스별 시간대**(cron 매시 정각 + `config_json.timezone` 현지 0시/12시 게이트 — 게이트 미통과 run 은 DB 조회 1회 후 1초 내 종료). 첫 실행 0건 디버깅: web search 응답 JSON 이 max_tokens 절단 → 8000 + 절단 복구 파서(Plan 16장 디버깅 후기). lint·build PASS. 상세 Plan 16장.) · 2026-06-11 (**PPT 본문 디자인 품질 일괄 + 데모 폴리시** — 상세 Plan 16장 2026-06-11. ① **본문 5종 디자인 개선**(bullets·agenda·twoCol·metric·quote, Cover/BackCover/타이틀/하단바 보존): 결정적 텍스트 측정 헬퍼(`estTextWidth`/`estLines`/`fitTextSize`) 공용화, `centeredTop` 수직 중앙 정렬(bullets/agenda/metric/quote), bullets L0 24 bold+폭 1200+**그리는 마커**(`bulletMarkerBox`), agenda 인덱스 14 bold 블루 eyebrow, 신규 `color.surface` 토큰으로 metric 카드 패널·twoCol 패널+칩(분할선 제거)·quote 좌측 블루 바(`quoteLayout`), twoCol/quote 오토핏. `agendaRowY(i, count)` 시그니처 변경. ② **PPT 개별 수정 묶음**: cover 저자날짜 18px·수상배지 85%/x120 정렬·보안칩 70%/우하단, 표지 부제목 2줄 클리핑 수정(미리보기 `overflow:visible`+SlideCanvas 경계 클리핑, §8 함정), bullets 간격 의미 반전(L1 은 부모에 44, 새 그룹 위 72), image 다이어그램 수직중앙+노드별 연한 배경/진한 테두리(`color.diagram.node` 5쌍), BackCover 수상배지 80%, metric 중앙정렬+`metricValueSize` 오토핏+프롬프트 value 8자 제약(zod 는 기존문서 호환 위해 미변경). ③ **UI/배포 수정**: 모바일 인터뷰 중앙정렬(ChatView 모바일 flex-col·lg 만 grid), 에이전트 헤더-버튼 `gap-6`, Slack 알림 URL 보강([notify.ts](src/lib/notify.ts) `\|\|` fallback+트레일링 슬래시 제거 — Vercel env `NEXT_PUBLIC_APP_URL` 빈 문자열이 localhost 링크 원인). lint·build PASS.) · 2026-06-10 (**Vercel 배포 + 서버리스 호환 수정** — 앱이 Vercel 에 배포됨(`penta-mind5.vercel.app`). 상세 Plan 16장·§8 함정. ① `/api/inngest` 500 디버깅 → 서버리스 Node-비호환 라이브러리 2종 교체: **jsdom→linkedom**([html.ts](src/lib/crawler/html.ts), Readability DOM), **pdf-parse→unpdf**([pdf.ts](src/lib/crawler/pdf.ts), pdfjs-dist v5 `DOMMatrix` 미정의 크래시). [next.config.ts](next.config.ts) `serverExternalPackages` 비움. ② **Node 22 고정**([package.json](package.json) `engines.node` + Vercel Node.js Version). ③ **함수 리전 서울 고정**([vercel.json](vercel.json) `icn1` — DB `ap-northeast-2` 정합, 페이지 지연 해소). ④ Inngest Cloud 앱 sync(Vercel 통합·배포 시 자동), env 점검(`NEXT_PUBLIC_APP_URL`·`INNGEST_*`·`INNGEST_DEV` 미설정), Deployment Protection off(앱은 Auth.js 보호). ⑤ 검증: `/api/inngest`=401(정상), Inngest Runs 동작, KB 크롤→ready 흐름 확인. **메모: 로컬 dev 와 prod 가 같은 Supabase 공유** → 기존 ready 소스는 dev 크롤분(청크 실재). 배포 중 올린 `crawling` 3건은 미완료 → 재업로드 필요. lint·build PASS.) · 2026-06-10 (**PPT 생성·보안레벨 묶음 — 3건 통합** / 상세 Plan 16장·[PLAN_A 문서](docs/PLAN_A_agenda_content_alignment.md)) · **① Plan A — agenda↔본문 정합**: [outline.ts](src/lib/prompts/outline.ts) `propose_outline`→**`propose_plan`**(`sections:[{title,kind}]`+`includeQuote`+풀버전 `sectionTitles`), [generate.ts](src/lib/ppt/generate.ts)가 `bodyBudget` 산정→`normalizeSections`(정확히 target개)→**agenda.items=토픽 제목**(코드 직접 조립)→토픽별 KB 검색(title 쿼리)→`fillSlideOnce`에 `forcedSlideTitle` 강제. 결과: agenda 항목수==본문 토픽수·텍스트==슬라이드 title. `PLAN_KINDS`=bullets/twoCol/metric/image, `AGENDA_MAX`=9. · **② 인터뷰 보안레벨 선택**: 마지막 `length` 단계 UI를 드롭다운 2개(분량+보안 Level1~5)로([ChatView](src/components/chat/ChatView.tsx)), 진행칩 라벨 "분량 및 보안 레벨"([machine.ts](src/lib/interview/machine.ts)), 보안레벨 클라 state→[finalize](src/app/api/interview/finalize/route.ts)→`generateDeck`→`meta.securityLevel`(하드코딩 1 제거). · **③ PPT 디자인**: 푸터 하단바 kind별 색(agenda `#F2F2F2`·section `#D9D9D9`·나머지 `#000000`, [tokens.ppt.json](src/design/tokens.ppt.json) `footerMaster.body.barFill`+[layouts.ts](src/lib/ppt/layouts.ts) `footerBarFill`/`footerIsDarkBar`), 검정 바에서만 보안칩=`_dark`·로고=`penta_white_small`. 슬라이드 **제목색 `#0060A9`**(신규 토큰 `color.title`, 7개 `role:'title'`). **cover 우측하단 보안칩**(`coverMaster.securityChip`, non-dark). **section 디바이더는 풀버전(16장+)만**(`distributeEvenly`, 목차 미포함). 미리보기·다운로드 동시 반영([render.tsx](src/lib/ppt/render.tsx)·[pptx.ts](src/lib/ppt/pptx.ts)). |
...
- pptxgenjs·unpdf·mammoth·linkedom 등 Node API 의존 라우트는 `export const runtime = 'nodejs'` 강제 (Edge X).
...
- **Vercel 서버리스 = Node-비호환 라이브러리 지뢰밭 → serverless 전용 대체로 교체, `serverExternalPackages` 는 현재 비움(`{}`)** → 과거 `jsdom`·`pdf-parse` 를 externalize 했으나 둘 다 Vercel 런타임에서 크래시(로컬 dev/`next start` 는 통과 → **배포 후에만 드러남**)라 제거. Node API 의존 라우트는 여전히 `export const runtime = "nodejs"` 강제. 새 무거운 native/DOM 의존 라이브러리는 **serverless 호환 여부를 먼저 확인**하고, 안 되면 대체를 찾는다. 사례:
  - **`jsdom` → `linkedom`**: jsdom 의 transitive `html-encoding-sniffer@6`(ESM-only)가 externalize+`require()` 경로에서 `ERR_REQUIRE_ESM` 으로 `/api/inngest` import 크래시. [html.ts](src/lib/crawler/html.ts) 의 Readability DOM 을 `linkedom` `parseHTML` 로 공급(`new Readability(document)`). cheerio 는 DOM 스펙이 아니라 Readability 대체 불가. linkedom 은 번들 친화적.
  - **`pdf-parse`(→pdfjs-dist v5) → `unpdf`**: pdfjs-dist v5 가 브라우저 전역 **`DOMMatrix`** 를 참조 → 서버리스 Node 엔 없어 `ReferenceError: DOMMatrix is not defined` 으로 크래시(GET만 하던 로컬 `next start` 는 PDF 파싱 코드 미평가라 안 터지고, Vercel external import eager 평가에서 터짐). [pdf.ts](src/lib/crawler/pdf.ts) 를 `unpdf`(`getDocumentProxy`/`extractText`/`getMeta`, Node 호환 pdfjs 내장)로 교체.
  - **Node 22 고정**: [package.json](package.json) `engines.node: "22.x"` + Vercel Settings → Build and Deployment → Node.js Version 22.x. (지금은 externalize 패키지가 없어 `require(ESM)` 리스크는 줄었지만 로컬·배포 런타임 일치를 위해 유지.)
  - **진단 팁**: 로컬 `pnpm build && pnpm start` → `/api/inngest` 가 `{"message":"Unauthorized"}`(401)면 코드 정상. **시크릿 창에서도** Vercel 만 500 이면(=Deployment Protection 아님) 런타임 크래시 → Vercel **Logs** 탭에서 `/api/inngest` 의 **첫 에러줄**(어떤 모듈/전역이 문제인지)을 확인하는 게 가장 빠르다.
...
- **.pptx 는 버전당 Storage 캐시 — 레이아웃 변경 시 `PPT_LAYOUT_REV` +1 필수** → 다운로드 라우트([pptx/route.ts](src/app/api/generate/[versionId]/pptx/route.ts))가 첫 생성 후 `pptx_object_key` 캐시를 재사용한다. 미리보기는 slidesJson 을 현행 레이아웃으로 매번 그리므로, 레이아웃/토큰을 바꾸면 "미리보기는 새 디자인·다운로드는 옛 디자인" 으로 어긋난다. 캐시 키에 [layouts.ts](src/lib/ppt/layouts.ts) `PPT_LAYOUT_REV` 가 `-r{N}.pptx` 로 들어가며 키 불일치 시 자동 재생성+옛 파일 삭제 — **PPT 출력이 달라지는 변경을 하면 반드시 REV 를 +1** 할 것.
- **PPT 미리보기 vs pptx 텍스트 동작 차이 — 클리핑·측정** → ① PowerPoint 텍스트 박스는 내용을 클리핑하지 않으므로 미리보기도 `overflow: visible`(박스) + SlideCanvas 캔버스 div 만 `overflow: hidden`(슬라이드 경계) — 박스에 `hidden` 을 다시 넣으면 미리보기만 잘려 "다운로드는 멀쩡한데 미리보기가 깨지는" 증상 재발. ② pptx 쪽엔 텍스트 측정 API 가 없다 — 줄수/폭 기반 동적 레이아웃은 반드시 [layouts.ts](src/lib/ppt/layouts.ts) 의 `estTextWidth`/`estLines`/`fitTextSize`(전각 1em·반각 0.6em 추정)를 **양 렌더러가 공유**해서 계산할 것. 한쪽만 실측(DOM measure 등)하면 미리보기≠pptx 로 어긋난다.
...
- **disabled 요소엔 hover 이벤트가 없다 — 툴팁은 래퍼에** → disabled 버튼/스위치에 Tooltip(또는 title)을 직접 달면 안 뜬다. 트리거는 래퍼 span([PresenterLockTooltip](src/components/PresenterLockTooltip.tsx))에 달고, 내부 요소는 `pointer-events-none` 이어야 hover 가 래퍼로 전달된다(shadcn Button 은 `disabled:pointer-events-none` 기본, raw `<button>` 은 직접 추가 필요).
- **외부 SDK 클라이언트는 import 시점 throw 금지 — 지연 초기화** → [anthropic.ts](src/lib/anthropic.ts)·[embeddings.ts](src/lib/embeddings.ts) 가 과거 모듈 top-level 에서 `if(!key) throw` 했는데, 그러면 그 모듈을 (transitive 포함) import 하는 페이지/라우트가 **키 없으면 전부 크래시**한다(UI-only 데모처럼 키를 지우는 순간 빌드·런타임 모두 깨짐). 규칙: 키 검증은 **실제 호출 시점**으로 미룬다 — anthropic 은 `export const anthropic` 을 Proxy 로 감싸 첫 속성 접근 때 클라이언트 생성(키 확인), embeddings 는 키 검증을 `callVoyage()` 내부로 이동. 새 외부 SDK 래퍼도 동일 패턴(Proxy/lazy getter)으로 추가할 것. storage.ts(Supabase)는 항상 켜진 서비스라 top-level throw 유지해도 무방.
- **UI-only 데모 모드 = `DEMO_UI_ONLY=1`** → 발표·데모 종료 후 비용/만료 서비스(Anthropic·Voyage·Inngest, +Sentry)를 끊고 기존 데이터로 화면만 둘러볼 때 켠다([demo-mode.ts](src/lib/demo-mode.ts) `UI_ONLY = process.env.DEMO_UI_ONLY === "1"`). 켜지면 인터뷰 질문=고정([service.ts](src/lib/interview/service.ts) `CANNED_QUESTIONS`)·문서 생성=고정 덱([generate.ts](src/lib/ppt/generate.ts) `cannedPlan`/`cannedSlide`)·Inngest 발화=no-op([client.ts](src/inngest/client.ts) `dispatch`). **정확히 `"1"` 일 때만** 모드 진입 — `0`/빈값/미설정은 모두 실서비스 복원(단 키도 다시 채워야 함, env 변경 후 재시작 필요). Supabase·Google 로그인은 유지해야 기존 데이터가 보인다. Sentry 만 [next.config.ts](next.config.ts) `withSentryConfig` 래퍼 제거로 빌드에서 분리(런타임 init 은 `SENTRY_DSN`/`NEXT_PUBLIC_SENTRY_DSN` env 가드).
...
2. **Demo B** — `scripts/force-change.ts` 로 시드 URL 강제 변경 → **감지** 후 우측 승인 카드(소스 변경·변경 비율) 생성·대기 → **발행 승인** 클릭 → `인식→판단→행동→학습` 잔여 단계가 피드에 진행되며 **자동 발행 + Slack `#docmind-demo` 알림** (승인 거부 시 갱신 중단 + "거부" 뱃지).
