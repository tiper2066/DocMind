...
| **Voyage AI** | 쿼리·청크 임베딩 (voyage-3, 1024차원) | 결제 등록으로 표준 한도 사용. 앱 측에서도 쿼리 캐시(LRU 256) + 배치 호출로 생성당 호출 17→4회 절감 |
...
- LLM 장애 대비 사전생성 인터뷰 fallback 캐시는 미구현 — 백업은 `/demo/playback` 녹화 재생
